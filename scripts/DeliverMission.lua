...
function DeliverMission:new(giverName, giverHome, originStation, destinationStation, cortex)
    ...
    o.originStation = origin
    o.destinationStation = destination
...
function DeliverMission:update(delta)
  ...
  if (self.state == self.STATE_HEADING_TO_ORIGIN) and (self.browncoat.ship:docked(self.origin) then
    self.giverHome:sendCommsMessage(
      self:browncoat.ship, 
      string.format(
  ...
  if (self.state == STATE_HEADING_TO_DESTINATION) and (self.browncoat.ship:docked(self.destination) then
    self.giverHome:sendCommsMessage(
      self:browncoat.ship, 
      string.format(
